function startBirthdayMatrix({ canvasId = 'birthdayCanvas', onFinished = () => {} } = {}) {
  ...
  function resize() {
    ...
    // Giữ tỉ lệ 16:9, vừa với màn hình điện thoại
    const desiredRatio = 16 / 9;
    if (vw / vh > desiredRatio) {
      H = vh;
      W = vh * desiredRatio;
    } else {
      W = vw;
  ...
  function setTargetsFromText(text) {
    ...
    const fontSize = Math.min(100, W / 10); // scale font theo width
    octx.font = `600 ${fontSize}px ${FONT_FAMILY}`;
    octx.shadowColor = 'rgba(255,182,193,0.9)'; octx.shadowBlur = 12;
    octx.fillText(text, W / 2, H / 2 + 50);
  ...
  function spawnBalloon() {
    return {
      type: 'balloon', x: Math.random() * W, y: H + 200,
      r: 18 + Math.random() * 10, vx: (Math.random() - 0.5) * 0.4, vy: -(0.6 + Math.random() * 0.8),
      hue: (300 + Math.random() * 120) | 0, wob: Math.random() * Math.PI * 2, wobAmp: 10 + Math.random() * 12
    };
  }
  function spawnMiniCake() {
    return {
      type: 'cake', x: Math.random() * W, y: H + 200,
      size: 18 + Math.random() * 10, vx: (Math.random() - 0.5) * 0.3, vy: -(0.5 + Math.random() * 0.7),
